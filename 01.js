...
function getTheAnswer( data ) {
    for ( let s = 0; s < data.length; s++ ) {
        for ( let l = data.length - 1; l >= 0; l-- ) {
            const smallNum = data[s];
            const largeNum = data[l];
            if ( largeNum < smallNum ) {
                break;
            }
            if ( largeNum + smallNum === 2020 ) {
                console.log( `Found ${ smallNum } + ${ largeNum } === 2020; ${ smallNum } × ${ largeNum } = ${ smallNum * largeNum }` );
...
console.log( getTheAnswer( data ) );


function getTheMoreComplicatedAnswer( data ) {
    for ( let s = 0; s < data.length; s++ ) {
        for ( let m = data.length - 1; m >= 0; m-- ) {
            ...
            for ( let l = data.length - 1; l >= 0; l-- ) {
                ...
                if ( medNum + smallNum + largeNum === 2020 ) {
                    console.log( `Found ${ smallNum } + ${ medNum } + ${ largeNum } === 2020; ${ smallNum } × ${ medNum } × ${ largeNum } = ${ smallNum * medNum * largeNum }` );
                    return largeNum * smallNum * medNum;
                }
            }
        }
    }
...
console.log( getTheMoreComplicatedAnswer( data ) );
